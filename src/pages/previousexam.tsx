...
const extractTagsFromCategoryLine = (rawMarkdown: string): string[] => {
    ...
    return [...categoryLine.matchAll(/#([a-z0-9-]+)/gi)].map((match) => match[1].toLowerCase());
};
...
const parseVaultQuestion = (sourcePath: string, rawMarkdown: string): PreviousExamQuestion | null => {
    const normalizedPath = sourcePath.replace(/\\/g, "/");
    const lines = rawMarkdown.split(/\r?\n/);

    const year = normalizedPath.match(/\/philnits-vault\/(\d{4})\//)?.[1] ?? "Unknown";
    ...
    const headingId = heading.match(/^#\s+([A-Za-z0-9._-]+)/)?.[1] ?? "unknown-question";
...
const parsedQuestions = Object.entries(vaultMarkdownModules)
    .map(([sourcePath, rawMarkdown]) => parseVaultQuestion(sourcePath, rawMarkdown))
    .filter((entry): entry is PreviousExamQuestion => Boolean(entry));

const groupedQuestionsByYear = Object.entries(
    parsedQuestions.reduce<Record<string, PreviousExamQuestion[]>>((accumulator, question) => {
    ...
    .map(([year, questions]) => ({
        year,
        questions: [...questions].sort((first, second) => compareQuestionNumbers(first.questionNumber, second.questionNumber)),
    }))
    .sort((first, second) => second.year.localeCompare(first.year, undefined, { numeric: true }));
                                    ...
                                    <section key={entry.year} className="border-2 border-black/20">
                                        ...
                                        >
                                            <div className="flex items-baseline gap-3">
                                                <h2 className="text-xl md:text-2xl font-bold">{entry.year}</h2>
                                                <p className="text-sm md:text-base opacity-70">{entry.questions.length} questions</p>
                                            </div>
                                            <CaretDownIcon
                                                            ...
                                                            >
                                                                <div className="flex flex-col gap-1">
                                                                    <p className="text-sm md:text-base font-bold">
                                                                        Q{question.questionNumber} · {question.examCode} · {question.subjectCategory}
                                                                    </p>
                                                                    <div className="text-sm md:text-base leading-relaxed prose prose-sm max-w-none prose-p:my-1">
                                                                        ...
                                                                        >
                                                                            {normalizeMathDelimiters(normalizeLegacySymbols(question.questionText))}
                                                                        </ReactMarkdown>
                                                                    </div>
                                                                    <p className="text-xs md:text-sm opacity-70">Topic: {question.subjectTopic}</p>
                                                                </div>
                                                                <CaretDownIcon
                                                            ...
                                                            {isQuestionOpen ? (
                                                                <div className="border-t border-black/10 px-3 pb-4 md:px-4">
                                                                    {question.options.length > 0 ? (
                                                                        <div className="pt-3 flex flex-col gap-2">
                                                                            ...
                                                                            {question.options.map((option) => (
                                                                                <div key={`${question.id}-${option.key}`} className="text-sm md:text-base">
                                                                                    ...
                                                                                    <div className="inline-block align-top prose prose-sm max-w-none prose-p:my-0">
                                                                                        <ReactMarkdown
                                                                                            ...
                                                                                            rehypePlugins={[rehypeKatex]}
                                                                                        >
                                                                                            {normalizeMathDelimiters(normalizeLegacySymbols(option.text))}
                                                                                        </ReactMarkdown>
                                                                                    </div>
                                                                                </div>
                                                                            ))}
                                                                    ...
                                                                    <div className="mt-3 border-l-4 border-green-700 pl-3">
                                                                        <p className="text-xs uppercase tracking-wide opacity-70">Correct Answer</p>
                                                                        ...
                                                                        <div className="text-sm md:text-base mt-1 leading-relaxed prose prose-sm max-w-none">
                                                                            {question.answerExplanation ? (
                                                                                <ReactMarkdown
                                                                                    ...
                                                                                    rehypePlugins={[rehypeKatex]}
                                                                                >
                                                                                    {normalizeMathDelimiters(normalizeLegacySymbols(question.answerExplanation))}
                                                                                </ReactMarkdown>
                                                                            ) : (
                                                                                <p>No detailed explanation available in the source markdown for this question.</p>
                                                                            )}
                                                                        </div>
                                                                    </div>
                                                                </div>
